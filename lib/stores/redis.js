...
function Redis (opts) {
  ...
  var redis = opts.redis || require('ioredis')
    , RedisClient = redis, Cluster = redis.Cluster;

  // initialize a pubsub client and a regular client
  if ((RedisClient && (opts.redisPub instanceof RedisClient)) || (Cluster && (opts.redisPub instanceof Cluster))) {
    ...
    this.pub = new redis(opts.redisPub.port || 6379, opts.redisPub.host || "localhost", opts.redisPub);
  }
  if ((RedisClient && (opts.redisSub instanceof RedisClient)) || (Cluster && (opts.redisSub instanceof Cluster))) {
    ...
    this.sub = new redis(opts.redisSub.port || 6379, opts.redisSub.host || "localhost", opts.redisSub);
  }
  if ((RedisClient && (opts.redisClient instanceof RedisClient)) || (Cluster && (opts.redisClient instanceof Cluster))) {
    ...
    this.cmd = new redis(opts.redisClient.port || 6379, opts.redisClient.host || "localhost", opts.redisClient);
  }
...
Redis.prototype.subscribe = function (name, consumer, fn) {
  ...
  if (consumer || fn) {
    ...
    self.sub.on('subscribe', subscribe); // <<-- NOTE: ioredis doesn't call this event anymore unlike plain redis

    //
    // these lines must happen (start)
    // 
    ...
    /*
     * Start listening to unsubscribe events
     */    
    ...
    self.sub.removeListener('subscribe', subscribe); // <<-- only handle the first call of subscribe event. This is to avoid multiple registration of event listener.

    fn && fn(); // callback?

    //
    // these lines must happen (end)
    // 
  }
